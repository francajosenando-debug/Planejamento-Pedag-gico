    ...
    <div className="space-y-6 pb-16 animate-in fade-in duration-300">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            {currentDay.lessons.length === 0 ? (
              <div className="py-8 text-center text-xs text-slate-400">
                Nenhuma aula cadastrada para {DAY_LABELS[activeDayKey]}. Clique em "Adicionar Aula" acima ou use a IA.
              </div>
            ) : (
              <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-700 pb-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg hover:bg-red-100 text-slate-400 hover:text-red-600 dark:hover:bg-red-950/40"
                          title="Excluir aula"
                        >
      ...
      {showImportLessonModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 max-w-lg w-full max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">Selecione uma Aula do Banco</h3>
              <button onClick={() => setShowImportLessonModal(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>

            <div className="flex-1 overflow-y-auto py-3 space-y-2">
              {savedLessons.length === 0 ? (
                <div className="py-6 text-center text-xs text-slate-400">Nenhuma aula salva no banco.</div>
              ) : (
                savedLessons.map((l) => (
                  <div
                    key={l.id}
                    onClick={() => handleImportSavedLesson(l)}
                    className="p-3 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-purple-50 dark:hover:bg-purple-950/40 cursor-pointer text-xs space-y-1 transition-colors"
                  >
                    <div className="font-bold text-slate-900 dark:text-white">{l.name}</div>
                    <div className="text-slate-500">{l.subject} • {l.theme}</div>
                  </div>
                ))
              )}
            </div>
